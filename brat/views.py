    ...
    if request.method == 'POST':
        ...
        if form1.is_valid() and form2.is_valid():
            if request.FILES:
                ...
                source_file = os.path.join(settings.UPLOAD_FILE_DIRS,
                                           file_name + '.txt')

                with open(source_file, 'wb+') as f:
                    for chunk in uf.chunks():
                        f.write(chunk)
                cmd = '/usr/bin/python2.7 /var/www/html/project/bratAdapter/bratShell.py %s %s %s' % (
                    source_file, file_name, folder_name)
                os.system(cmd)
...
def setchecked(request, folder_name):
    os.system(
        'python /var/www/html/project/setChecked/setChecked.py /var/www/html/brat/data/unchecked/%s copener'
        % folder_name)
