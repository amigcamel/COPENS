...
class SearchForm(forms.Form):
    query = forms.CharField(
        widget=forms.TextInput(
            attrs={
                'required': 'required',
                'placeholder': u"請輸入關鍵字",
                'id': 'namanyay-search-box',
                'size': 40
            }),
        min_length=1,
        max_length=300,
        error_messages={'required': '請輸入關鍵字'})
    database = forms.MultipleChoiceField(
        widget=forms.CheckboxSelectMultiple,
        choices=DB_CHOICE,
        error_messages={'required': '請至少選擇一項'})


class ColloForm(forms.Form):
    algorithms = forms.ChoiceField(
        choices=[('chi_sq', 'chi_sq'), ('likelihood_ratio',
                                        'likelihood_ratio'), ('pmi', 'pmi'),
                 ('student_t', 'student_t'), ('dice', 'dice'),
                 ('mi_like', 'mi_like'), ('poisson_stirling',
                                          'poisson_stirling'),
                 ('jaccard', 'jaccard'), ('phi_sq', 'phi_sq'), ('raw_freq',
                                                                'raw_freq')],
        label="演算法",
        help_text="選擇欲使用之演算法")
    stopword_filter = forms.IntegerField(
        min_value=1,
        max_value=500,
        required=False,
...
class ConcForm(forms.Form):
    pos = forms.BooleanField(
        required=False, label=("顯示詞類"), help_text="在結果中顯示詞性，若取消勾選，搜尋速度較快")
    window_size = forms.IntegerField(
        min_value=1,
        max_value=10,
        initial=8,
        widget=forms.NumberInput(attrs={'onkeydown': "return false"}),
        label="詞語索引左右顯示數量",
        help_text="關鍵字之左/右顯示之字數")
    sampling = forms.BooleanField(
        required=False, label=('隨機抽樣'), help_text="針對所有結果進行隨機抽樣,點選進行抽樣數目調整")
    ...
    display_num = forms.IntegerField(
        ...
        label=("每頁顯示筆數"),
        help_text="每頁顯示詞語索引數量")


class SketchForm(forms.Form):
    min_logdice = forms.FloatField(
        required=False,
        label=("Minimal logdice value"),
        widget=forms.NumberInput(attrs={'placeholder': '可為空白'}))
    min_occ = forms.IntegerField(
        required=False,
...
class LopeRegForm(RegistrationForm):
    ...
    email = forms.EmailField(
        ...
        help_text="僅可使用gmail, hotmail, yahoo 或.edu 網域 (例：foo@ntu.edu.tw) 申請註冊")
